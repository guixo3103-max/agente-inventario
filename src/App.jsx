...
const REQUIRED_FIELDS = [
  ...
  { key: "m8",  label: "Mes 8" },
  { key: "m9",  label: "Mes 9" },
  { key: "m10", label: "Mes 10" },
  { key: "m11", label: "Mes 11" },
...
const calcMinimo = (abc, c) => {
  ...
  if (abc === "A00") return Math.ceil(c * 1.5 + 2.33 * c * Math.sqrt(0.25));
...
const processRaw = (rows, mapping) => rows.map(row => {
  ...
  const base = {
    bodega: String(g("bodega")||""), articulo: String(g("articulo")||""),
...
export default function App() {
  ...
  const autoMap = useCallback(() => {
    const hints = {
      ...
      stock:       ["stock_bod","stockbod","existencia total","stockbodega"],
      stock_cd:    ["pm122","stockcd","stock_cd","cd"],
    ...
    REQUIRED_FIELDS.forEach(({ key }) => {
      ...
      const match = headers.find(h => {
        ...
        return kws.some(kw => {
          const kn = kw.toLowerCase().replace(/[^a-z0-9]/g,"");
          return hn === kn || hn.includes(kn);
        });
      });
  ...
  if (step==="map") return (
    <div style={{minHeight:"100vh",background:"#F8F7F4",padding:"2rem 1.5rem",...S}}>
      <div style={{maxWidth:700,margin:"0 auto"}}>
        ...
        <div style={{background:"white",border:"0.5px solid #D3D1C7",borderRadius:12,padding:"1rem 1.25rem",marginBottom:20,maxHeight:"60vh",overflowY:"auto"}}>
          {REQUIRED_FIELDS.map(({key,label})=>(
            <div key={key} style={{display:"flex",alignItems:"center",gap:12,padding:"6px 0",borderBottom:"0.5px solid #F1EFE8"}}>
              <div style={{minWidth:200,fontSize:12,color:"#444441"}}>{label}</div>
  ...
  const DetailModal = ({ r, onClose }) => {
    const maxVenta = Math.max(...r.meses, 1);
    return (
      <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.45)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000,padding:"1rem"}}>
        <div style={{background:"white",borderRadius:16,width:"100%",maxWidth:900,maxHeight:"90vh",overflowY:"auto",padding:"1.5rem"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:16}}>
            ...
            <button onClick={onClose} style={{fontSize:18,border:"none",background:"none",cursor:"pointer",color:"#888780",padding:"0 8px"}}>✕</button>
          </div>
          ...
          {/* Recomendación */}
          <div style={{background:TIPO_CONFIG[r.tipo].bg,borderRadius:8,padding:"0.8rem 1rem",marginBottom:16,display:"flex",alignItems:"center",gap:10}}>
          ...
          <div style={{marginBottom:8}}>
            <div style={{fontSize:11,color:"#888780",textTransform:"uppercase",letterSpacing:"0.07em",marginBottom:10}}>Ventas últimos 12 meses (mes 1 = más reciente)</div>
            <div style={{display:"flex",gap:4,alignItems:"flex-end",height:80}}>
              {r.meses.map((v,i)=>(
                <div key={i} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:3}}>
                  <div style={{fontSize:10,color:"#888780"}}>{v||""}</div>
                  <div style={{width:"100%",background:v>0?"#185FA5":"#F1EFE8",borderRadius:"3px 3px 0 0",height:`${Math.max(4,(v/maxVenta)*56)}px`,transition:"height 0.2s"}}/>
  ...
  return (
    <div style={{minHeight:"100vh",background:"#F8F7F4",...S}}>
      ...
      <div style={{padding:"1.25rem 1.5rem"}}>
        ...
        <div style={{fontSize:11,color:"#888780",marginBottom:10}}>{filtered.length.toLocaleString()} de {metrics.total.toLocaleString()} registros · Clic en una fila para ver detalle</div>
            ...
            <div style={{overflowX:"auto"}}>
              <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
                <thead><tr style={{background:"#F8F7F4"}}>
                  {["Bodega","SKU","Descripción","ABC","Consumo","Stock","Tránsito","Posición","Mínimo","Máximo","Sugerido","Acción"].map(h=><th key={h} style={{padding:"8px 10px",textAlign:"left",fontSize:11,color:"#888780",fontWeight:500,borderBottom:"0.5px solid #D3D1C7",whiteSpace:"nowrap"}}>{h}</th>)}
                </tr></thead>
                <tbody>
                  {filtered.slice(0,200).map((r,i)=>{
                    const tc=TIPO_CONFIG[r.tipo];
                    return <tr key={i} onClick={()=>setSelected(r)} style={{borderBottom:"0.5px solid #F1EFE8",background:r.tipo==="CRITICO"?"#FFF8F8":"white",cursor:"pointer"}} onMouseEnter={e=>e.currentTarget.style.background="#F8F7F4"} onMouseLeave={e=>e.currentTarget.style.background=r.tipo==="CRITICO"?"#FFF8F8":"white"}>
                      <td style={{padding:"7px 10px",maxWidth:120,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{r.bodega}</td>
          ...
          <div style={{background:"white",border:"0.5px solid #D3D1C7",borderRadius:12,overflow:"hidden"}}>
            <div style={{overflowX:"auto"}}>
              <table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}>
                <thead><tr style={{background:"#F8F7F4"}}>
                  {["SKU","Descripción","ABC","Stock\nBodega","Stock\nCD","Tránsito","Consumo","Mínimo","Máximo","M1","M2","M3","M4","M5","M6","M7","M8","M9","M10","M11","M12","Recomendación"].map((h,i)=>(
                    <th key={i} style={{padding:"6px 8px",textAlign:i>=9&&i<=20?"right":"left",fontSize:10,color:"#888780",fontWeight:500,borderBottom:"0.5px solid #D3D1C7",whiteSpace:"pre-wrap",lineHeight:1.3}}>{h}</th>
                  ))}
                </tr></thead>
                <tbody>
                  {filtered.slice(0,200).map((r,i)=>{
                    ...
                    return <tr key={i} onClick={()=>setSelected(r)} style={{borderBottom:"0.5px solid #F1EFE8",background:r.tipo==="CRITICO"?"#FFF8F8":"white",cursor:"pointer"}} onMouseEnter={e=>e.currentTarget.style.background="#F8F7F4"} onMouseLeave={e=>e.currentTarget.style.background=r.tipo==="CRITICO"?"#FFF8F8":"white"}>
                      <td style={{padding:"6px 8px",fontFamily:"monospace",whiteSpace:"nowrap",fontSize:11}}>{r.articulo}</td>
                      <td style={{padding:"6px 8px",maxWidth:150,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",color:"#444441"}} title={r.descripcion}>{r.descripcion}</td>
                      <td style={{padding:"6px 8px"}}><span style={{background:r.abc_bodega==="A00"?"#FCEBEB":r.abc_bodega==="A"?"#EAF3DE":"#F1EFE8",color:r.abc_bodega==="A00"?"#A32D2D":r.abc_bodega==="A"?"#3B6D11":"#5F5E5A",padding:"1px 6px",borderRadius:3,fontSize:10,fontWeight:500}}>{r.abc_bodega}</span></td>
                      <td style={{padding:"6px 8px",textAlign:"right",fontWeight:500}}>{r.stock}</td>
                      <td style={{padding:"6px 8px",textAlign:"right",color:"#185FA5"}}>{r.stock_cd}</td>
                      <td style={{padding:"6px 8px",textAlign:"right",color:"#888780"}}>{r.transito}</td>
                      <td style={{padding:"6px 8px",textAlign:"right"}}>{r.consumo}</td>
                      <td style={{padding:"6px 8px",textAlign:"right",color:"#854F0B"}}>{r.minimo}</td>
                      <td style={{padding:"6px 8px",textAlign:"right",color:"#3B6D11"}}>{r.maximo}</td>
                      {r.meses.map((v,mi)=>(
                        <td key={mi} style={{padding:"4px 8px",textAlign:"right",position:"relative"}}>
                          <div style={{position:"relative",display:"flex",flexDirection:"column",alignItems:"flex-end",gap:2}}>
                            <span style={{fontSize:11,color:v>0?"#2C2C2A":"#D3D1C7"}}>{v||"—"}</span>
                            <div style={{width:"100%",height:3,background:"#F1EFE8",borderRadius:2}}>
                              <div style={{width:`${(v/maxM)*100}%`,height:"100%",background:"#185FA5",borderRadius:2}}/>
                            </div>
                          </div>
                        </td>
                      ))}
                      <td style={{padding:"6px 8px",whiteSpace:"nowrap"}}><span style={{background:tc.bg,color:tc.color,padding:"2px 6px",borderRadius:3,fontSize:10,fontWeight:500}}>{tc.label}</span></td>
                    </tr>;
                  })}
                </tbody>
              </table>
